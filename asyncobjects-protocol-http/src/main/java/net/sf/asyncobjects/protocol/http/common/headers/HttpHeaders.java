package net.sf.asyncobjects.protocol.http.common.headers; // NOPMD
...
import static net.sf.asyncobjects.protocol.LineUtil.isBlank;
import static net.sf.asyncobjects.protocol.LineUtil.writeLatin1;
...
public final class HttpHeaders {
    ...
    public static Promise<HttpHeaders> readHeaders(final ByteParserContext input, final int limit) { // NOPMD
        ...
        return aSeqLoop(new ACallable<Boolean>() {
            ...
            @Override
            public Promise<Boolean> call() throws Exception { // NOPMD
    ...
    private List<String> getHeadersList(final String name) {
        ...
        List<String> list = headers.get(normalizedName);
        if (list == null) {
            list = new LinkedList<>();
            headers.put(normalizedName, list);
        }
        return list;
    }
