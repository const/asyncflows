#!/bin/bash
export PATH=$PATH:/C/utils/Java/jdk1.6.0_24/bin
...
echo '01' > serial
touch index.txt
openssl req -new -x509 -passout pass:passphrase -extensions v3_ca -keyout private/cakey.pem -out certs/cacert.pem -days 7305 -config caconfig.conf -subj "/C=US/ST=NV/L=Area 51/O=Hangar 18/OU=X.500 Standard Developers/CN=Test Root CA"
...
# create server
keytool -genkeypair -alias localhost -keyalg RSA -keysize 2048 -sigalg SHA256withRSA -dname "C=US, ST=NV, L=Area 51, O=Hangar 18, OU=X.500 Standard Developers, CN=Localhost Server" -keypass passphrase -validity 7305 -storetype JKS -keystore server.jks -storepass passphrase
keytool -certreq -alias localhost -sigalg SHA256withRSA -file certs/localhost.req.pem -keypass passphrase -storetype JKS -keystore server.jks -storepass passphrase
...
# create client
keytool -genkeypair -alias client -keyalg RSA -keysize 2048 -sigalg SHA256withRSA -dname "C=US, ST=NV, L=Area 51, O=Hangar 18, OU=X.500 Standard Developers, CN=Local Client" -keypass passphrase -validity 7305 -storetype JKS -keystore client.jks -storepass passphrase
keytool -certreq -alias client -sigalg SHA256withRSA -file certs/localclient.req.pem -keypass passphrase -storetype JKS -keystore client.jks -storepass passphrase
